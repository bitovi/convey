...
export function useDragToPlace({
  ...
  const transitionToCrossFrame = useCallback((e: PointerEvent) => {
    ...
    if (target) {
      try {
        target.postMessage({ __conveyDrag: true, type: 'DRAG_START', componentName: name, storyId: sid, ghostHtml: ghostHtml ?? undefined, ghostCss: ghostCss ?? undefined, componentPath: cPath, args: cArgs, screenX: e.screenX, screenY: e.screenY, clientX: e.clientX, clientY: e.clientY }, '*');
      } catch { /* ignore */ }
    }
    onStart?.();
  ...
  const crossFrameMoveHandler = useCallback((e: PointerEvent) => {
    const target = getTarget();
    if (target) {
      try { target.postMessage({ __conveyDrag: true, type: 'DRAG_MOVE', screenX: e.screenX, screenY: e.screenY, clientX: e.clientX, clientY: e.clientY }, '*'); } catch { /* ignore */ }
    }
  }, [getTarget]);
  ...
  const crossFrameUpHandler = useCallback((e: PointerEvent) => {
    ...
    const target = getTarget();
    if (target) {
      try { target.postMessage({ __conveyDrag: true, type: 'DRAG_END', screenX: e.screenX, screenY: e.screenY, clientX: e.clientX, clientY: e.clientY, cancelled: false }, '*'); } catch { /* ignore */ }
    }
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (phaseRef.current === 'panel') {
        ...
        const target = getTarget();
        if (target) {
          try { target.postMessage({ __conveyDrag: true, type: 'DRAG_END', screenX: 0, screenY: 0, cancelled: true }, '*'); } catch { /* ignore */ }
        }
        latestRef.current.onDragEnd?.();
        cleanup();
      }
